...
const EditEventModal = ({ open, onClose, eventData, onSave }) => {
  const [title, setTitle] = useState('');
  const [url, setUrl] = useState(''); // Добавлено поле URL
  const [description, setDescription] = useState('');
  const [date, setDate] = useState('');
  const [time, setTime] = useState('');
  const [endDate, setEndDate] = useState('');
  ...
  useEffect(() => {
    if (eventData) {
      setTitle(eventData.title);
      setUrl(eventData.url || ''); // Загружаем URL из данных события
      setDescription(eventData.description);
      setDate(new Date(eventData.date).toISOString().split('T')[0]);
      setTime(
        new Date(eventData.date)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedEvent = {
        ...eventData,
        title,
        url, // Добавляем URL в обновленные данные
        description,
        date: new Date(`${date}T${time}`).getTime(),
        dateEnd: new Date(`${endDate}T${endTime}`).getTime(),
        prices: formattedPriceOptions,
      ...
      // Обработка ошибки дублирования URL
      if (
